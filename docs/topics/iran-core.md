...
من برای نت ثابت مخابرات و شاتل موبایل، این DNS سرور جدیده رو پیشنهاد می‌کنم تست کنید:

```rust
https://dns.malw.link/dns-query
```

و یا DoHاختصاصی گروه **[Xstack][XS]** رو، چون واقعا خفنه

```rust
https://rustdns.devefun.org/dns-query
```

لیست سایت‌ها و سرویس‌هایی که می‌تونه تحریمشون رو دور بزنه خیلی خیلی زیاده و واقعا هم چیزای جذابی تو لیستش هست بجز اینستاکرام و توئیتر و ... 
