...
- حتماً تبلیغات را تماشا کنید، در غیر این صورت ممکن هنگام استفاده از توربوتل با مشکل مواجه شوید. اگر هر روز تبلیغات را تماشا نکنید، نمی‌توانید پروفایل کاربرهای دیگر را باز کنید.

:::
</Ltr>

باشه چشم اصغر آقا شما امر کن، فقط لطفا یکم آهسته‌تر پیش برید که بتونم نوت‌برداری کنم :writing_hand: عررررررر
...
- [هرچیزی که نیازه درباره فابل‌های Cache تلگرام بدونیم](#کش)
  - [کش‌ها دقیقا چی هستن؟؟](#کش-چیه)
  - [انواع کش در تلگرام](#انواع-کش)
    - [کش مدیا - (Media Cache)](#کش-مدیا)
    - [کش متن - (Local Database)](#کش-دیتابیس)
  - [چرا باید کش رو پاک کنیم؟؟](#دلایل-حذف-کش)
  - [نحوه پاکسازی کش مدیا](#پاکسازی-کش-مدیا)
    - [روی موبایل (اندروید و iOS)](#روی-موبایل)
    - [روی دسکتاپ (ویندوز و مک](#روی-دسکتاپ)
    - [پاکسازی خودکار کش‌ها](#پاک-سازی-خودکار)
  - [نحوه پاکسازی کش لوکال دیتابیس](#پاکسازی-کش-دیتابیس)
  - [جمع‌بندی مطالب مرتبط با Cache](#جمع-بندی)
...
تنها کاری که لازمه بکنید یه **Clean Install** کردنه، یه نصب تر و تازه. جزئیات خاصی نداره و این چند سطر رو برای دوستان سالخورده خودم می‌نویسم چون ممکنه حواسشون نباشه. شما حرفه‌ای‌ها غلط کردید بامن که بخونید اصلا لزومی نداره که بخونید توضیحات راه‌حل اول رو :smirk: :facepunch:

### نصب تلگرام موقت {#تلگرام-موقت}
...
**از طریق ایجاد فایل بکاپ از تنظیمات توربو**

همونطور که گفتم مشکل ما مغایرت تنظیمات اضافی توربوتل بین نسخه‌های `v11` و `v12` بودش، کاری که می‌گم رو انجام بدید.
...
### مدیریت فایل اندروید {#مدیریت-فایل}
...
[این کـد XML](#بکاپ) رو کپی کنید [^1] بعد برگردید سراغ **MT**.

داخل فایل تو یه جایی از صفحه دوبار ضربه بزنید تا گزینه‌های بیشتر نمایش داده بشن.
...
# هرچیزی که نیازه درباره فایل‌های کش تلگرام بدونیم {#کش}

وقتی از تلگرام استفاده می‌کنیم، برنامه برای اینکه سرعتش بالا باشه و هر بار لازم نشه همه‌چیز رو از اینترنت دانلود کنه، میاد و یه سری فایل‌های کوچولوی بند‌انگشتی موقتی روی حافظه گوشی یا کامپیوتر ذخیره می‌کنه. به این فایل‌ها می‌گیم **Cache** یا همون حافظه پنهان.

## کش‌ها دقیقا چی هستن؟ {#کش-چیه}

**کش در واقع یه جور حافظه موقته**

مثلا، وقتی یه عکس یا مثلا [این ویدیوی لانا][Lana] رو رو توی تلگرام باز می‌کنی، اون ویدئو روی حافظه دستگاهت پنهونی ذخیره می‌شه تا چندروز بعد که دوباره رفتی کانال و از کنار ویدئوی لانا گذشتی و دلت خواست دوباره ببینیش دیگه این‌بار منتظر نمونی که دانلود بشه، همون‌روز اولین‌بار که بازش کردی دانلود شده و یه گوشه از حافظه دستگاه پنهون شده تا این‌بار خیلی سریع‌ بجای دانلود شدن از سرور تلگرام از حافظه گوشیت باز بشه.
...
- **پس کش باعث می‌شه که
   تجربه کاربری روون‌تر باشه، اما اگه همین کش زیاد بشه می‌تونه حافظه گوشی رو قشنگ پر کنه و بخاطر همینه که گاها می‌کنی تلگرامت داره سنگین‌کار می‌کنه و هنگ و لگ داره.

## انواع کش در تلگرام {#انواع-کش}

**تلگرام دو نوع کش اصلی داره.**

### 1. کش مدیا (Media Cache) {#کش-مدیا}
...
- المان‌های اضافی اکانت‌های لاشیای پول‌دار (پرمیوم)

این‌ها معمولا بیشترین حجم رو می‌گیرن. مثلا اگه توی یه گروه باشی که هر دقیقه ویدئو و موزیک و غیره می‌فرستن و توام همه رو باز می‌کنی و به هیچی نه نمیگی، کش مدیات می‌تونه تا چند گیگابایت از حافظه دستگاه رو اشغال کنه.

### 2. کش متنی یا دیتابیس محلی (Local Database) {#کش-دیتابیس}
...
::: danger **نحوه عملکرد Local DataBase Cache**

::: details مشاهده توضیحات
...
- آزاد کردن فضای ذخیره‌سازی گوشی، تبلت یا کامپیوتر
- رفع برخی باگ‌ها و مشکلات نرم‌افزاری برنامه
- افزایش حریم خصوصی  
  (مثلا وقتی نمی‌خوای هیچ فایل‌ و ردپایی روی دستگاهت بمونه)
...
- وارد **Settings** بشو.
- برو به بخش **Data and Storage**.
- روی **Storage Usage** بزن.
- اینجا می‌تونی ببینی هر دسته از فایل‌ها  
  (عکس، ویدیو، اسناد و...) چقدر فضا گرفتن.  
- برای پاک کردن، روی **Clear Cache** بزن.  
...
### روی دسکتاپ (ویندوز و مک‌بوک) {#روی-دسکتاپ}
- وارد **Settings** بشو.
- برو به بخش **Advanced**.
- گزینه **Manage Local Storage** رو انتخاب کن.
- حالا می‌تونی کش مدیا رو پاک کنی.

## نحوه پاک کردن کش متنی یا لوکال دیتابیس {#پاکسازی-کش-دیتابیس}
- وارد **Settings** بشو.
- برو به بخش **Data and Storage**.
- روی **Storage Usage** بزن.
- روی آیکون سه نقطه بالای صفحه در گوشه سمت راست (منوی انگلیسی) و بالا سمت چپ (منوی فارسی) بزن.
- روی **Clear Local DataBase** بزن.
- روی **Clear** بزن و چند ثانیه صبر کن تا تموم بشه.
...
و بعد روی **Add exception** برنید و مورد دل‌خواهتون رو به لیست اضافه کنید تا هیچ‌گونه فایل کش از اون به‌طور خودکار پاک نشه.

<br/>
<p align="center">
  <img src="/turbotel/12.png" alt="Slider" width="1080" height="596" />
</p>
:::

<br/>

- یکم پایینتر تو همون صفحه‌ی **Storage Usage** یه بخشی بود که اسلایدر داشت، منظورم **`Maximum Cache Size`** بود، اسلایدر رو می‌تونیم از پنج گیگ تا حالت نامحدود تغییر بدیم، عقل و منطق میگه بازم هرکس نسبت به نیازش باید تعیین کنه، من میگم همون حداقل مقدار 5GB خیلی‌هم عالیه.

<br/>
<p align="center">
  <img src="/turbotel/13.png" alt="Slider" width="1080" height="484" />
</p><br/>
...
[^1]: گوشه سمت راستش آیکون کپی هست بزنید روش کپی میشه.
